...
\begin{figure}[!t]
	\centering
		\includegraphics[width=\columnwidth]{fig/regComp.eps}
...
We also extend this approach to register many other types of objects and demonstrate a wide range of applications in areas such as image super-resolution. 
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=\columnwidth]{fig/theory_example.eps}
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=.9\columnwidth]{fig/error_prop.eps}
...
In this section, we show results on two applications of the proposed method, including facial AU detection and image super-resolution. In addition, we have also included qualitative results on different types of objects to demonstrate the generalizability of our approach.
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=.8\columnwidth]{fig/suppliment.eps}
...
Since the ground-truth label is only available for the FERA-AU training set, we carry out a person-independent cross validation experiment, where no test subject is used for training, and the average performance is reported. Due to the finite scale of the training exemplars, person-independent test is essential to demonstrate the generalization ability of an approach to unseen subjects. Table~\ref{table:fera} shows the performance obtained using both LBP and LBP-TOP feature extractors, respectively. The area under curve (AUC) score of the receiver operating characteristic (ROC) curve is reported. As seen from Table~\ref{table:fera}, on average, SOFIT outperforms the other methods by a significant margin using both LBP and LBP-TOP features. SOFIT achieves the best performance in 9 out of 12 AU classification tasks. In general, LBP-TOP out-perform LBP likely due to the dynamic information extracted. EAI is on par with PA in terms of average AUC score. EAI performs well in categorical emotion recognition~\cite{Yang_SMCB12}; in a per-frame based setting, however, EAI lacks ability to maintain the subtle muscle motion. Moreover, dynamic feature is not advantageous in the EAI case. A plausible explanation is that the inherent dynamics are buried in the blockiness artifacts caused by SIFT flow. We should point out that the video sequences in FERA-GEMEP is relatively short, \textit{i.e.} around 2 seconds. Thus, the propagation error is negligible and thus we exclude the loop closure rectification procedure in this experiment. 

\noindent \textbf{Datasets: BP4D}
We also carry out experiments on the BP4D~\cite{BP4D} dataset. It is a spontaneous facial expression dataset collected in an lab setting with uncontrolled pose and gesture. It contains 41 subjects participating in 8 tasks, which are designed to solicit expressions that are not deliberately posed. The subjects are aging from 18 to 29 covering various ethnicity groups. The original dataset includes both 2D and 3D videos, and we only use the 2D videos in our experiment. Similar to the FERA-GEMEP dataset, the group truth label of each frame is obtained according to the FACS. 

The person-independent experimental setup is the same as in~\cite{BP4D}, where we follow a 10-fold cross-validation procedure and use 39 subjects for training and 2 for testing. We adopt the aforementioned preprocessing and feature extraction steps. The experimental results for BP4D is tabulated in Table~\ref{table:bp}. Similar observation can be made as in the FERA dataset. SOFIT achieves the best score in most AU detection tasks and the dynamic feature improves the performance except for the EAI case. 


\begin{table*}[htbp]
\caption{Person-independent AUC-score result on BP4D dataset (2D data only)}
\begin{center}
\label{table:bp}
\begin{tabular}{ccccccccccccccc}
\toprule
...
\midrule
\multirow{2}{*}{BaseLine~\cite{Valstar_FERA11}}	& LBP 		&0.53 	&0.55 	&0.47 	&0.59 	&0.49 	&0.55 	&0.59 	&0.50 	&0.59 	&0.56 	&0.49 	&0.53 	&0.54 \\
																								& LBP-TOP &0.54 	&0.61 	&0.49 	&0.61 	&0.48 	&0.55 	&0.54 	&0.51 	&0.60 	&0.57 	&0.51 	&0.55 	&0.55 \\
																								\midrule
\multirow{2}{*}{PA~\cite{Tadas_ECCV14}}					& LBP 		&0.58 	&0.61 	&0.49 	&0.62 	&0.49 	&0.61 	&0.61 	&0.54 	&0.59 	&0.61 	&0.55 	&0.57 	&0.57 \\
																								& LBP-TOP &0.57 	&0.58 	&0.52 	&0.64 	&\textbf{0.57}\cellcolor[gray]{0.9} 	&0.60 	&0.58 	&0.52 	&0.64 	&0.60 	&\textbf{0.59}\cellcolor[gray]{0.9} 	&0.61 	&0.58 \\
																								\midrule
\multirow{2}{*}{EAI~\cite{Yang_SMCB12}}					& LBP 		&0.61 	&0.62 	&0.55 	&0.60 	&0.55 	&0.61 	&0.65 	&\textbf{0.55}\cellcolor[gray]{0.9} 	&0.60 	&0.60 	&0.50 	&0.52 	&0.58 \\
																								& LBP-TOP &0.63 	&0.58 	&0.55 	&0.61 	&0.48 	&0.61 	&0.60 	&0.50 	&0.62 	&0.58 	&0.57 	&0.52 	&0.57 \\
																								\midrule				
\multirow{2}{*}{SOFIT}													& LBP 		&0.57 	&\textbf{0.65}\cellcolor[gray]{0.9} 	&\textbf{0.56}\cellcolor[gray]{0.9} 	&0.64 	&0.48 	&0.62 	&0.60 	&\textbf{0.55}\cellcolor[gray]{0.9} 	&0.59 	&0.61 	&0.54 	&0.65 	&0.59 \\
																								& LBP-TOP &\textbf{0.65}\cellcolor[gray]{0.9} 	&0.63 	&0.56 	&\textbf{0.71}\cellcolor[gray]{0.9} 	&0.53 	&\textbf{0.67}\cellcolor[gray]{0.9} 	&\textbf{0.66}\cellcolor[gray]{0.9} 	&0.53 	&\textbf{0.68}\cellcolor[gray]{0.9} 	&\textbf{0.66}\cellcolor[gray]{0.9} 	&0.58 	&\textbf{0.66}\cellcolor[gray]{0.9} 	&\textbf{0.63}\cellcolor[gray]{0.9} \\
\bottomrule
\end{tabular}
\end{center}
\end{table*}
...
\begin{figure}[t]
	\centering
		\includegraphics[width=\columnwidth]{fig/superResolution.eps}
...
\begin{figure}[t]
	\centering
		\includegraphics[width=.8\columnwidth]{fig/SR_quant.eps}
...
\begin{figure*}[htbp]
	\centering
		\includegraphics[width=.65\textwidth]{fig/object_ex.eps}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{pic/mingyang.jpg}}]{Mingyang Li} received B.Eng. degree in Automation Engineering from University of Electronic Science and Technology of China, Chengdu, China, in 2009, and Ph.D. degree in
