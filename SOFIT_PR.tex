...
\author[SCU_ECE]{Yinjie Lei\corref{mycorrespondingauthor}}
\author[Google]{Mingyang Li}
\author[UCR]{Ninad Thakoor}
\author[UCR]{Bir Bhanu}

\cortext[mycorrespondingauthor]{Corresponding author. Email: yinjie@scu.edu.cn}
...
\begin{algorithm}[htb]
	...
	\begin{algorithmic}[1]
		\For{$i=0$ to $N$}
		\If{ $i\%K=0$ }
		\State{$init \leftarrow True$}
		\EndIf
		\If{ $init$ }
		\State{$f_s^i \leftarrow $SIFT-flow$(I_i,I_{ref})$}
		\State{$T_i \leftarrow f_s^ip^T(pp^T)^{-1}+\I_3$ (Eq.~\ref{eq:result})}
		\Else
		\State{$h \leftarrow \min(i,H)$}
		\For{ $j=1$ to $i$ }
		\State{$f_o^{i-j,i} \leftarrow$ optical-flow$(I_{i-j},I_i)$ }
		\EndFor
		\State{$g \leftarrow f_s^{i-1}-f_s^{i-h-1}+\sum_{j=1}^h(f_o^{i-j,i}-f_o^{i-j-1,i-1})$}
		\State{$T_i \leftarrow T_{i-1}+(1+\alpha+\beta)^{-1}(f_o^{i-1,i}+\frac{\alpha}{h}g) + \I_3$ (Eq.~\ref{eq:dp})}
		\EndIf
		\State{$init \leftarrow False$}
		\EndFor
	\end{algorithmic}
	\textbf{Output:} $T_{0:N}$. \\
	\label{alg}
